...
import io.github.ximin.xlake.writer.FileFormat;
...
public sealed interface DataBlock<K extends Comparable<K>>
        permits
        DataBlock.MemoryMappedDataBlock,
    ...
    // todo 这里得区分是本地的文件还是hadoop文件的路径，所以参数类型最好是string
    // 检查数据块是否与指定的文件位置相关
    ...
    record FileDataBlock<K extends Comparable<K>>(
            Range<K> range,
            long size,
            Path filePath,
            long blockSize,
